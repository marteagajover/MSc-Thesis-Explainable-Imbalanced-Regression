## DIBS selects the strategy that is applied to generate new synthetic examples 
## according to the distances between them. The two candidate strategies to
## obtain new examples are "SmoteRegress" and "GaussNoiseRegress". The selection 
## procedure takes into account the distances distribution. If both examples are 
## far away, then "GaussNoise" is selected, otherwise, "SmoteRegress" is used.


DIBSRegress <- function(form, dat, method = method, npts = npts, 
                 control.pts = control.pts, thr.rel = 0.5,
                 C.perc = "balance", k = 5, repl = FALSE,
                 dist = "Euclidean", p = 2, pert)
  
  # INPUTS:
  # form        a model formula
  # dat         the original training set (with the unbalanced distribution)
  # method      a parameter previously defined for the relevance function
  # npts        a parameter previously defined for the relevance function
  # control.pts a parameter previously defined for the relevance function
  # thr.rel     the relevance threshold above which a case is considered
  #             to belong to the rare "class"
  # C.perc      list containing the percentage of under- or/and over-sampling 
  #             to apply to each "class" obtained for that threshold.
  #             The oversampling percentage means that the examples above the 
  #             threshold are increased by this percentage. The undersampling
  #             percentage means that the normal cases (those below the threshold)
  #             are undersampled by this percentage. Alternatively, it may be
  #             "balance" or "extreme", cases where the sampling percentages
  #             are automatically estimated.
  # k           number of neighbors to consider as the pool from which the new 
  #             synthetic examples are generated when using SmoteR strategy
  # repl        if it is allowed to perform sampling with replacement for SmoteR strategy
  # dist        distance measure to use (defaults to "Euclidean") to obtain new
  #             examples through SmoteR strategy
  # p           a parameter used when a p-norm is computed
...

{
  ...
  if (is.list(C.perc)) {
    if (length(C.perc) != nbump){
      ...
      # estimate the percentages of over/undersampling
...
DIBSRegress.exs <- function(orig, ind, tgt, N, k, dist, p, pert)
  ...
  # N       the percentage of oversampling to carry out
  # k       the number of nearest neighours
  # dist    the distance function used for the neighours computation
  # p       an integer used when a "p-norm" distance is selected
  # pert    the perturbation introduced when applying GaussNoiseRegress strategy 
